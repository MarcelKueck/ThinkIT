...
while i < 100:
    ...
    if len(line) > 20:
        ...
        if meditationInt >30 and meditationInt<70 and not flag:
            flag=True
            color_print('Start', color='green', bold=True)
            ser.write('f')
        if attentionInt > 50 and attentionInt<80 and flag:
            color_print('Left', color='blue')
            ser.write('l')
        if attentionInt<50 and attentionInt >30 and flag:
            color_print('Right', color='blue')
            ser.write('r')
        if meditationInt>70 and flag:
            ser.write('s')
            flag=False
